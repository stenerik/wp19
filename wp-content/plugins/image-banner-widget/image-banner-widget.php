...
Plugin URI: https://metinsaylan.com/wordpress/plugins/image-banner-widget/
Description: Add Images and Banners to your sidebars easily! Add title, link, alternative text, even set the link targets with one widget. No coding required!
Version: 1.4.5
Author: Metin Saylan
Author URI: https://metinsaylan.com/
*/

define('SHAILAN_IBW_VERSION','1.4.4');

// Register widget
add_action( 'widgets_init', 'wpa_register_ibw' );
function wpa_register_ibw(){
	register_widget("shailan_BannerWidget");
}
...
if(!class_exists('shailan_BannerWidget')){
	class shailan_BannerWidget extends WP_Widget {
		...
		public function __construct(){
			...
			$widget_ops = array(
				'classname' => 'shailan_banner_widget',
				'description' => __( 'Image only banner', 'image-banner-widget' )
			);

			parent::__construct(
				'banner',
				__('Image / Banner Widget', 'image-banner-widget'),
				$widget_ops
			);
